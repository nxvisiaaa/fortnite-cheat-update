...
{
	DWORD_PTR bonearray;
	bonearray = vmread<DWORD_PTR>(mesh + 0x5E8);

	if (bonearray == NULL)
	{
		bonearray = vmread<DWORD_PTR>(mesh + 0x5F8);  //(mesh + 0x5e8) + 0x5a));
	}
...
void ActorLoop()
{
	while (true)
	{
		...
		settings::Uworld = vmread<DWORD_PTR>(baseaddy + 0xEE3BA38);
		...
		for (int i = 0; i < settings::ActorCount; ++i) {
			...
			float player_check = vmread<float>(CurrentActor + 0x4468);
			if (player_check == 10) {
...
void blackballs()
{
	...
	for (FNlEntity entity : entityList) {
		...
		Vector3 Headpose = GetBoneWithRotation(mesh, 106);
		...
		Vector3 vHeadBone = GetBoneWithRotation(mesh, 106);
		...
		uintptr_t CurrentWeapon = vmread<uintptr_t>(settings::LocalPawn + 0x8f8); //FortPawn::CurrentWeapon 0x868
